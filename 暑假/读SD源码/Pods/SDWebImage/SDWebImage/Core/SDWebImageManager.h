...
//学习这个类
//用于表示一个“组合操作”，即图片加载操作中涉及到的缓存查询操作和网络加载操作的结合。这个类实现了 SDWebImageOperation 协议，主要用于管理和控制图片加载的整个过程，包括缓存查询和图片下载。
@interface SDWebImageCombinedOperation : NSObject <SDWebImageOperation>

// 具备了取消操作（cancel）的能，并且可以被其他部分的代码用来统一管理图片加载操作。
- (void)cancel;

// cacheOperation 是一个属性，用于保存缓存查询的操作对象。这个操作是指从缓存中查找图片的过程。
// 它是 readonly 的，意味着一旦设置，就不能在外部修改。
// 如果图片已经在缓存中存在，cacheOperation 会负责将图片从缓存中读取出来，而无需进行网络下载。
@property (strong, nonatomic, nullable, readonly) id<SDWebImageOperation> cacheOperation;

// loaderOperation 是另一个属性，用于保存图片加载的操作对象，通常是下载操作。
// 同样，它是 readonly 的。
// 如果图片不在缓存中，loaderOperation 会负责从网络上下载图片。
